...
import { View, Text, StyleSheet, Image, TouchableOpacity, TextInput, Alert, ScrollView, ToastAndroid, RefreshControl } from 'react-native';
...
export default function index(props) {
    ...
    const [shimmerArr, setshimmerArr] = useState(["1", "2", "3", "4", "5"]);
    ...
    const handleStatusVoucher = async () => {
        ...
        setTimeout(async () => {
            ...
            Service.updateStatusVoucher(idPromo, 2).then(res => {
                console.log("handleStatusVoucher -> res", res)
                console.log("handleStatusVoucher -> res", res.status)
                if (res.status === 200) {
                    console.log("handleStatusVoucher -> res", res.status)
                    getVoucherApi();
                    setTimeout(() => {
    ...
    const getVoucherApi = async () => {
        await AsyncStorage.getItem('xxTwo').then((res) => {
            Service.getVoucherAktif(JSON.parse(res).id_toko)
                .then(result => {
                    if (result.status === 200) setVouchers(result.voucher) & setVoucher(result.voucher);
                    setTimeout(() => setshimmer(false), 1000);
                })
                .catch(e => {
                    setshimmer(false)
                    setTimeout(() => ToastAndroid.show(e, ToastAndroid.LONG, ToastAndroid.CENTER), 500);
                });
        }).catch(e => {
            setshimmer(false)
            setTimeout(() => ToastAndroid.show(e, ToastAndroid.LONG, ToastAndroid.CENTER), 500);
                        ...
                        text: "YA", onPress: () => {
                            ...
                            Service.deleteVoucher(idPromo).then(res => {
                                if (res.status === 200) {
                                    ...
                                    setspinner(false)
                                    setTimeout(() => ToastAndroid.show(res.status.message, ToastAndroid.LONG, ToastAndroid.CENTER), 250);
                                }
                            }).catch(err => {
                                setspinner(false)
                                setTimeout(() => ToastAndroid.show(err, ToastAndroid.LONG, ToastAndroid.CENTER), 250);
                            })
                            actionSheetRef.current?.setModalVisible(false);
                        }
                    }
                    ...
                    voucherApi.length === 0 ?
                        ...
                        shimmer ?
                                    ...
                                    <Card key={item} style={styles.card}>
                                        <View style={[style.row_space_top, styles.cardHead]}>
                                            <ShimmerPlaceHolder
                                                ...
                                                style={[styles.cardImage, { marginRight: '11%' }]}
                            ...
                            voucherApi.map(product => {
                                // console.log("🚀 ~ file: index.js ~ line 252 ~ index ~ product", product)
                                return (
                                        ...
                                        <View style={style.row_space}>
                                            <View style={styles.cardText}>
                                                ...
                                                {product.tipe_diskon === "persentase" ?
                                                    ...
                                                    <Text style={styles.cardStok}>Diskon : Rp {money(String(product.potongan_diskon))}</Text>
                                                }
                                                {product.kategori !== null ? <Text style={styles.cardStok}>Khusus Kategori : {product.kategori} - {product.sub_kategori}</Text> : null}
                                            </View>
                                            <Card.Actions>
                                                <TouchableOpacity onPress={() => {
                                                    console.log("🚀 ~ file: index.js ~ line 271 ~ index ~ onPress", product)
                                                    setshowAlert(false)
            ...
            <ActionSheet containerStyle={styles.actionSheet} ref={actionSheetRef}>
                ...
                <View style={styles.modalLine}>
                    ...
                    <View style={styles.abc}>
                        <Text style={styles.textLine}>{isEnabled ? "Aktif" : "Tidak Aktif"}</Text>
                        <Switch
