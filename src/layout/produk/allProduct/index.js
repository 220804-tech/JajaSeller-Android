...
import { Text, View, Image, TouchableNativeFeedback, TouchableOpacity, Dimensions, FlatList, Alert, ScrollView, RefreshControl, ToastAndroid, TouchableHighlight } from 'react-native';
...
import { ServiceProduct, Utils, Colors, useNavigation, Wp, Hp, ShimmerProduct, Style, ActionSheetProduct } from '../../../export';
...
export default function index(props) {
    ...
    const reduxAllProducts = useSelector(state => state.product.allProducts)
    const reduxFetchProducts = useSelector(state => state.product.fetchAllProduct)
    ...
    const [settingvariasi, setsettingVariasi] = useState(false);
    ...
    }, [props.params, reduxFetchProducts]);
    ...
    const handleFetchProduct = async () => {
        ...
        try {
            ...
            console.log("file: index.js ~ line 1002 ~ handleFetchProduct ~ error", error)
        }
    }
    ...
    const handleStatusProduk = async () => {
        ...
        actionSheetRef.current?.setModalVisible(false);
        setTimeout(() => {
            setitemPressed(idProduct)
        }, 155);

        setTimeout(() => {
            ServiceProduct.updateStatusProduct(idProduct, productOnpress.status_produk === "live" ? 2 : productOnpress.status_produk === "arsipkan" ? 1 : null)
                .then((res) => {
                    if (res.status === 200) {
                        handleFetchProduct();
                ...
                <Card key={item.id}
                    ...
                    <TouchableOpacity onPress={() => item.status_produk === "blokir" ? handleRevisi(item) : navigation.navigate("ProdukPreview", { data: item })} style={styles.cardItem}>
                        ...
                        <View style={styles.cardText}>
                            ...
                            <Text adjustsFontSizeToFit style={styles.cardHarga}>{String(item.variasi[0]?.harga_normal)} / <Text adjustsFontSizeToFit style={[styles.cardHarga, { color: item.jumlah_stok_in && item.jumlah_stok_in > 0 ? Colors.blackLight : Colors.redPower }]}>{item.jumlah_stok_in && item.jumlah_stok_in > 0 ? item.jumlah_stok_in : "Stok habis"}</Text></Text>

                            <Text adjustsFontSizeToFit style={[styles.status, { color: item.status_produk ? item.status_produk === "live" ? Colors.biruJaja : item.status_produk === "arsipkan" ? Colors.blackGrey : item.status_produk === "blokir" ? Colors.redPower : Colors.kuningJaja : Colors.white }]}>{item.status_produk ? item.status_produk === "arsipkan" ? 'diarsipkan' : item.status_produk === "blokir" ? "diblokir" : item.status_produk : null}</Text>
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            {reduxAllProducts && reduxAllProducts.length ?
            ...
            <ActionSheetListVariasi
                ...
                <View style={{ paddingHorizontal: Wp('2%'), marginBottom: '2%', height: Hp('77%'), }}>
            ...
            <ActionSheetLayout
                footerHeight={11}
                containerStyle={[styles.actionSheet, { height: Hp('45%') }]}
                ...
                <View style={[Style.column_0_start, { height: '80%', width: '100%' }]}>
                    <TouchableOpacity
                        onPress={() => handleEditVariasi()}
                        style={[Style.py_3, { borderBottomWidth: 0.2, borderBottomColor: Colors.silver, width: '100%' }]}
                    >
                        <Text style={[Style.font_13]}>Ubah Variasi</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        onPress={() => setalertHapusVariasi(true)}
                        style={[Style.py_3, { borderBottomWidth: 0.2, borderBottomColor: Colors.silver, width: '100%' }]}
                    >
                        <Text adjustsFontSizeToFit style={[Style.font_13]}>Hapus Variasi</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        onPress={() =>
                            actionSheetLayout.current?.setModalVisible(false) &
                        ...
                        style={[Style.py_3, { borderBottomWidth: 0.2, borderBottomColor: Colors.silver, width: '100%' }]}
                    >
                        <Text adjustsFontSizeToFit style={[Style.font_13]}>Atur Diskon</Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                        ...
                        style={[Style.py_3, { borderBottomWidth: 0.2, borderBottomColor: Colors.silver, width: '100%' }]}
                    >
                        <Text adjustsFontSizeToFit style={[Style.font_13]}>Hapus Diskon</Text>
                    </TouchableOpacity>
                </View>
            </ActionSheetLayout>
            <ActionSheetSettingVariasi
                ...
                <View style={[Style.column_0_center, { height: '100%' }]}>
                    <ScrollView>
