...
export default function index(props, { route }) {
  ...
  const reduxArchiveProducts = useSelector(state => state.product.archiveProducts)
  const reduxFetchArchive = useSelector(state => state.product.fetcharchiveProduct)
  ...
  }, [props.params, reduxFetchArchive]);
  ...
  const handleFetchProduct = async () => {
    ...
    setproductOnpress({})
    try {
      let archiveProduct = await ServiceProduct.fetchArchiveProduct(reduxSeller)
      if (archiveProduct) {
        dispatch({ type: 'SET_PRODUCTS_ARCHIVE', payload: archiveProduct })
        dispatch({ type: 'FETCH_ARCHIVE', payload: false })
      }
    } catch (error) {
      console.log("file: index.js ~ line 110 ~ handleFetchProduct ~ error", error)
    }
  }
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {reduxArchiveProducts && reduxArchiveProducts.length ?
