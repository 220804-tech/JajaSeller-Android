...
export default function index(props) {
  ...
  const reduxBlockedProducts = useSelector(state => state.product.blockedProducts)
  const reduxFetchBlocked = useSelector(state => state.product.fetchblockedProduct)
  ...
  const handleFetchProduct = async () => {
    ...
    setproductOnpress({})
    try {
      let blockedProducts = await ServiceProduct.fetchBlockedProduct(reduxSeller)
      if (blockedProducts && blockedProducts.length) {
        dispatch({ type: 'SET_PRODUCTS_BLOCKED', payload: blockedProducts })
        dispatch({ type: 'FETCH_BLOCKED', payload: false })
      }
    } catch (error) {
      console.log("file: index.js ~ line 107 ~ handleFetchProduct ~ error", error)
    }
  }
  ...
  const handleOptions = async (item) => {
    console.log("🚀 ~ file: index.js ~ line 192 ~ handleOptions ~ item", item.id_produk)
  ...
  const renderItem = ({ item }) => {
    console.log("🚀 ~ file: index.js ~ line 238 ~ renderItem ~ item", item)
    if (shimmerProduct === true) {
      return <ShimmerProduct />
    }
    else {
      return (
        <Card key={item.id_produk}
          ...
          <TouchableOpacity onPress={() => item.status_produk === "blokir" ? handleRevisi(item) : navigation.navigate("ProdukPreview", { data: item })} style={styles.cardItem}>
            ...
            <View style={styles.cardText}>
              ...
              <Text adjustsFontSizeToFit style={[styles.status]}>Status : <Text style={{ color: item.status_produk === "blokir" ? Colors.redPower : Colors.kuningJaja }}>{item.status_produk === "blokir" ? "diblokir" : item.status_produk}</Text></Text>
            </View>
          ...
          <Text numberOfLines={2} style={[Style.font_12, Style.italic, Style.px, { color: Colors.redNotif, backgroundColor: Colors.kuningWarning }]}>Note : {item.konfirmasi_produk[0].alasan}</Text>
        </Card>
      );
    }
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {reduxBlockedProducts && reduxBlockedProducts.length ?
