...
import { ServiceProduct, Utils, Colors, useNavigation, Wp, Hp, Loading, Style, Ws, ShimmerProduct, ActionSheetProduct } from '../../../export';
...
export default function index(props, { route }) {
  ...
  const reduxLiveProducts = useSelector(state => state.product.liveProducts)
  const reduxFetchLive = useSelector(state => state.product.fetchliveProduct)
  ...
  const handleFetchProduct = async () => {
    ...
    setproductOnpress({})
    try {
      let liveProducts = await ServiceProduct.fetchLiveProduct(reduxSeller)
      if (liveProducts) {
        dispatch({ type: 'SET_PRODUCTS_LIVE', payload: liveProducts })
        dispatch({ type: 'FETCH_LIVE', payload: false })
      }
    } catch (error) {
      console.log("file: index.js ~ line 104 ~ handleFetchProduct ~ error", error)
    }
  }
  ...
  const renderLabel = (label) => {
    return (
      <TouchableOpacity
        onPress={() => alert('ahah')}
        style={[Style.row_space0, Style.py_2, Style.my_2]}>
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {reduxLiveProducts && reduxLiveProducts.length ?
