...
import { Style, Colors, Wp, useNavigation } from '../../export'
...
export default function Income() {
    ...
    const reduxIncome = useSelector(state => state.wallet.income)
        ...
        <SafeAreaView style={[Style.container, { backgroundColor: Platform.OS === 'ios' ? Colors.white : null }]}>
            <DataTable>
                ...
                <ScrollView contentContainerStyle={{ paddingBottom: '40%' }}>
                    {reduxIncome && reduxIncome.length ?
                        reduxIncome.map((item, index) => {
                            return (
                                <DataTable.Row key={index} onPress={() => handlePress(item.invoice)}>
                                    <View style={[Style.column_0_center_start, { width: Wp('40%') }]}>
