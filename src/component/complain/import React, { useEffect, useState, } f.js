...
import { Colors, Hp, useNavigation } from '../../export';
export default function index(props) {
    ...
    const renderLabel = (label) => {
        return (
            <TouchableOpacity
                ...
                style={{ padding: '1%' }}
            >
                <Text style={{ fontSize: 14, color: Colors.blackGrey, fontFamily: 'Poppins-SemiBold' }}>{label}</Text>
            </TouchableOpacity>
        );
    };
    ...
    return (
        <SafeAreaView style={{ flex: 1, flexDirection: 'column', backgroundColor: Platform.OS === 'ios' ? Colors.biruJaja : null }} >
            ...
            {reduxAuth ?
                <ActionButton buttonColor={Colors.kuningJaja} renderIcon={() =>
                    <Image
                    ...
                    {/* <ActionButton.Item buttonColor={Colors.kuningJaja} title="Jaja Story" onPress={() => handleCloseAS("Jaja Story")}>
                    <Text style={styles.textMenu}>J</Text>
                </ActionButton.Item> */}
                    <ActionButton.Item buttonColor={Colors.biruJaja} title="Voucher Toko" onPress={() => handleCloseAS('Voucher Toko')}>
                ...
                <View style={style.column_center_start}>
                    <View style={[style.column_center_start, { width: '100%', marginBottom: '3%' }]}>
                        <Dropdown
                            ...
                            labelTextStyle={{ fontWeight: '900', fontSize: 14, color: Colors.blackLight }}
                            labelColor={Colors.biruJaja}
                            textColor={zonaWaktu ? Colors.blackgrayScale : Colors.silver}
                            labelFontSize="14"
                            data={waktuIndonesia}
                            value={zonaWaktu}
                    ...
                    <TouchableOpacity style={[style.column_center_start, { width: '100%', marginBottom: '11%' }]} onPress={showDatePicker}>
                        <TextInput
                            ...
                            style={[styles.inputBox, { color: date ? Colors.blackgrayScale : Colors.silver }]}
                            value={date}
                            theme={{
                                colors: {
                    ...
                    <View style={[style.column_center_start, { width: '100%', marginBottom: '3%' }]}>
                        <Button style={{ width: '100%' }} contentStyle={{ width: '100%' }} color={Colors.biruJaja} labelStyle={{ color: Colors.white }} mode="contained" onPress={() => console.log('Pressed')}>
                            Buat Flashsale
                        </Button>
                    </View>
                    <DateTimePickerModal
